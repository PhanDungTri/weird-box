...
import { PassiveActionInfo, PurifyInfo, SpellInfo } from "../../../../../shared/@types";
...
const Spells = ({ id, justifyContent = "center" }: SpellsProps): JSX.Element => {
  ...
  useListenServerEvent(SERVER_EVENT_NAME.Purify, (info: PurifyInfo) => {
    const { target, remove } = info;

    setSpells((list) =>
      produce(list, (draft) => {
        if (target === id) remove.forEach((r) => delete draft[r]);
      })
    );
  });
  ...
  useListenServerEvent(SERVER_EVENT_NAME.ActivatePassive, (passive: PassiveActionInfo) => {
    ...
    setSpells((list) =>
      produce(list, (draft) => {
        if (target === id && draft[passive.id]) draft[passive.id].strength = 0;
      })
    );
  });
  ...
  useListenServerEvent(SERVER_EVENT_NAME.NewTurn, () =>
    setSpells((list) =>
      produce(list, (draft) => {
        for (const id in draft) if (draft[id].duration === 0 || draft[id].strength === 0) delete draft[id];
      })
    )
  );
